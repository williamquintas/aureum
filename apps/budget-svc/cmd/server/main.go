...
import (
	...
	"github.com/aureum/budget-svc/internal/infrastructure/persistence"
...
func run() int {
	...
	if err := telemetry.InitOTEL("budget-svc", "1.0.0"); err != nil {
	...
	categoryRepo := persistence.NewCategoryRepo(dbPool)
	...
	outboxPublisher := outbox.NewPublisher(outboxStore, kafkaProducer, "budget-events", 5*time.Second)
	outboxPublisher.Start(ctx)

	var flagClient application.FeatureFlag
	if cfg.UnleashURL != "" && cfg.UnleashToken != "" {
		uc, err := ff.NewClient(cfg.UnleashURL, "budget-svc", cfg.UnleashToken)
		if err != nil {
	...
	svc := application.NewService(
		budgetRepo, categoryRepo, outboxRepo,
		idempStore, redisCache, flagClient,
	)
	...
	go func() {
		log.Info("budget-svc listening", "port", cfg.GRPCPort)
