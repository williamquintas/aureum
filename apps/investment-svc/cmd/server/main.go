...
import (
	...
	"github.com/aureum/investment-svc/internal/infrastructure/persistence"
...
func run() int {
	...
	if err := telemetry.InitOTEL("investment-svc", "1.0.0"); err != nil {
	...
	investmentRepo := persistence.NewInvestmentRepo(dbPool)
	...
	outboxPublisher := outbox.NewPublisher(outboxStore, kafkaProducer, "investment-events", 5*time.Second)
	outboxPublisher.Start(ctx)

	var flagClient application.FeatureFlag
	if cfg.UnleashURL != "" && cfg.UnleashToken != "" {
		uc, err := ff.NewClient(cfg.UnleashURL, "investment-svc", cfg.UnleashToken)
		if err != nil {
	...
	svc := application.NewService(
		investmentRepo, transactionRepo,
	...
	go func() {
		log.Info("investment-svc listening", "port", cfg.GRPCPort)
