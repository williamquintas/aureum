// Package main is the entry point for the credit card service.
package main
...
import (
	...
	"github.com/aureum/creditcard-svc/internal/infrastructure/api"
	"github.com/aureum/creditcard-svc/internal/infrastructure/persistence"
...
func run() int {
	...
	if err := telemetry.InitOTEL("creditcard-svc", "1.0.0"); err != nil {
	...
	creditCardRepo := persistence.NewCreditCardRepo(dbPool)
	invoiceRepo := persistence.NewInvoiceRepo(dbPool)
	transactionRepo := persistence.NewTransactionRepo(dbPool)
	...
	outboxPublisher := outbox.NewPublisher(outboxStore, kafkaProducer, "creditcard-events", 5*time.Second)
	outboxPublisher.Start(ctx)

	var flagClient application.FeatureFlag
	if cfg.UnleashURL != "" && cfg.UnleashToken != "" {
		uc, err := ff.NewClient(cfg.UnleashURL, "creditcard-svc", cfg.UnleashToken)
		if err != nil {
	...
	svc := application.NewService(
		creditCardRepo, invoiceRepo, transactionRepo,
	...
	creditcardv1.RegisterCreditCardServiceServer(grpcServer, handler)
	...
	go func() {
		log.Info("creditcard-svc listening", "port", cfg.GRPCPort)
...
func loadConfig() config {
	...
	if dbURL == "" {
		dbURL = "postgres://aureum:aureum@localhost:5432/creditcarddb" //nolint:gosec
	}
